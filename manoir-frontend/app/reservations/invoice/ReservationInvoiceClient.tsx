...
const parseReservationDate = (value?: string) => {
  ...
  // Une date seule est interprétée en heure locale pour éviter un décalage de
  // jour. Une date/heure ISO doit rester complète afin de conserver l'heure du
...
const documentNumber = (reservation: Reservation, type: DocumentType, payment?: Payment) => {
  ...
  if (type === "deposit") {
    return (
      reservation.deposit_invoice_number || payment?.invoice_number || `FAC-${year}-${sequence}`
    );
  }

  if (type === "stay") {
    return (
      reservation.stay_invoice_number || payment?.invoice_number || `FAC-SEJ-${year}-${sequence}`
    );
  }
...
export default function ReservationInvoicePage() {
  ...
  const documentType: DocumentType =
    requestedType && requestedType in documentLabels ? requestedType : "booking";

  const reservationId = searchParams.get("id") || (Array.isArray(params.id) ? params.id[0] : params.id);
  ...
  useEffect(() => {
    ...
    loadData();
  }, [authLoading, reservationId, router, user]);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-cream/70">Chargement du document...</p>
        </div>
      </div>
    );
  }
  ...
  const paidPayment = reservation.payments?.find(
    (payment) => payment.payment_type === paymentType && payment.status === "success"
  );
  ...
  const depositDays =
    depositRate > 0
      ? Math.round(depositAmount / depositRate)
      : diffDays(reservation.created_at, reservation.check_in);
  ...
  const totalNet = isCancellationDocument
    ? cancellationRefund
    : isDepositDocument
      ? depositAmount
      : stayAmount;
  ...
  const unitPrice = isCancellationDocument
    ? depositRate
    : isDepositDocument
      ? depositRate
      : stayUnitPrice;
  const quantity = isCancellationDocument
    ? cancellationDays
    : isDepositDocument
      ? depositDays
      : nights;
  ...
  const fallbackInvoiceDate =
    documentType === "stay" ? reservation.stay_paid_at : reservation.paid_at;
  const documentDate =
    isInvoice && paidPayment?.paid_at
    ...
    <div className="invoice-print-page min-h-screen py-28 md:py-32">
      <div className="mx-auto mb-4 max-w-4xl px-4 print:hidden">
        <div className="flex items-center justify-between">
          <Link
            href="/reservations"
            className={`flex items-center gap-2 ${theme.text} ${theme.hoverText}`}
          >
      ...
      <div className="invoice-print-document mx-auto max-w-4xl bg-white shadow-lg print:shadow-none">
        <div className={`border-b-4 p-8 ${theme.border} ${theme.header}`}>
          <div className="flex flex-col gap-8 md:flex-row md:items-start md:justify-between">
            ...
            <div className="md:text-right">
              <h2 className={`mb-3 text-3xl font-bold ${theme.text}`}>
              ...
              <div className="rounded-lg bg-white p-4 shadow-sm">
                ...
                <p className="mt-2 text-sm text-gray-600">
                  {isInvoice ? "Date et heure de délivrance" : "Date"}
                </p>
        ...
        <div className="grid gap-8 p-8 md:grid-cols-2">
          <div>
            <h3
              className={`mb-4 border-b-2 pb-2 text-lg font-semibold text-gray-900 ${theme.border}`}
            >
              Client
            </h3>
            ...
            <div className="space-y-2 text-gray-700">
              ...
              {isCancellationDocument && (
                <>
                  <p>
                    Date d'annulation :{" "}
                    {formatReservationDate(reservation.cancelled_at || new Date().toISOString())}
                  </p>
            ...
            <div className="grid gap-4 text-sm md:grid-cols-3">
              ...
              <div>
                ...
                <p className="font-semibold text-gray-900">
                  {documentType === "deposit" ? "Caution" : "Séjour"}
                </p>
              </div>
              <div>
                <p className="text-gray-600">Transaction</p>
                <p className="font-semibold text-gray-900">
                  {paidPayment?.transaction_id || "Non renseignée"}
                </p>
              </div>
            </div>
          </div>
        ...
        {!isInvoice && (
          <div className="px-8 pb-8">
            <div
              className={`rounded-xl border-2 p-6 text-sm leading-relaxed text-gray-700 ${theme.notice}`}
            >
              {documentType === "booking"
                ...
                : documentType === "cancellation"
                  ? "Ce bon confirme l'annulation de votre réservation et le montant à rembourser dans un délai de 48h à 72h."
